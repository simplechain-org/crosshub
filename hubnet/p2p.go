...
func (p2p *P2P) AsyncSend(addr *peer.AddrInfo, msg *Msg) error {
...
func (p2p *P2P) SendWithStream(s network.Stream, msg *Msg) error {
	return p2p.send(s, msg)
}

func (p2p *P2P) Send(addr *peer.AddrInfo, msg *Msg) (*Msg, error) {
	s, err := p2p.streamMng.get(addr.ID)
	if err != nil {
...
func AddrToPeerInfo(multiAddr string) (*peer.AddrInfo, error) {
	maddr, err := multiaddr.NewMultiaddr(multiAddr)
	if err != nil {
		return nil, err
	}
