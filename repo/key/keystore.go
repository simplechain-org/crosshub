...
import (
	...
	"github.com/simplechain-org/go-simplechain/crypto/sym"
...
func New(password string) (*Key, error) {
	...
	key, err := ecdsa.GenerateKey(ecdsa.Secp256k1)
	if err != nil {
...
func NewWithPrivateKey(privateKey crypto.PrivateKey, password string) (*Key, error) {
	...
	address := crypto.PubkeyToAddress(privateKey.(*ecdsa.PrivateKey).K.PublicKey)
	//if err != nil {
	//	return nil, err
	//}
...
func (key *Key) GetPrivateKey(password string) (crypto.PrivateKey, error) {
	...
	return ecdsa.UnmarshalPrivateKey(bytes, ecdsa.Secp256k1)
}
